...
export function ComparisonSection() {
  const comparisonItems = [
    ...
    },
    {
      ...
      traditional: "High exposure to denial audits and unverified AI hallucinations",
  ...
  return (
    <section id="why-notenra" className="py-24 sm:py-32 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-6 bg-slate-50 border border-slate-200 rounded-3xl p-8 space-y-6">
            <div className="flex items-center justify-between pb-6 border-b border-slate-200">
              <div>
                ...
                <h3 className="text-2xl font-bold text-slate-700">Traditional EHR Workflow</h3>
              </div>
            ...
            <div className="space-y-5">
              {comparisonItems.map((item, idx) => (
                <div key={idx} className="flex items-start gap-3 text-xs text-slate-600 pb-4 border-b border-slate-200/60 last:border-0 last:pb-0">
                  <XCircle className="w-4 h-4 text-rose-500 shrink-0 mt-0.5" />
                  <div>
                    <span className="font-bold text-slate-800 block mb-0.5">{item.metric}</span>
          ...
          <div className="lg:col-span-6 bg-surface-teal border border-[#4ACCCE]/45 rounded-3xl p-8 space-y-6 shadow-lg shadow-[#0E7C93]/10 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between pb-6 border-b border-[#0E7C93]/15 relative z-10">
              <div>
                <div className="text-xs font-extrabold text-[#0E7C93] uppercase tracking-widest mb-1">
                  Next-Gen Intelligence
                </div>
                <h3 className="text-2xl font-bold text-[#1B3F60]">NOTENRA Clinical Workflow</h3>
              </div>
              <div className="w-10 h-10 rounded-full bg-gradient-cyan text-white flex items-center justify-center shadow-md shadow-[#0E7C93]/25">
            ...
            <div className="space-y-5 relative z-10">
              {comparisonItems.map((item, idx) => (
                <div key={idx} className="flex items-start gap-3 text-xs text-slate-600 pb-4 border-b border-[#0E7C93]/10 last:border-0 last:pb-0">
                  <CheckCircle2 className="w-4 h-4 text-[#0E7C93] shrink-0 mt-0.5" />
                  <div>
                    <span className="font-bold text-[#1B3F60] block mb-0.5">{item.metric}</span>
