...
export function FaqSection() {
  ...
  return (
    <section id="faq" className="py-24 sm:py-32 bg-slate-50 border-t border-slate-200/60 relative overflow-hidden">
        ...
        <div className="space-y-4">
          {faqs.map((faq, idx) => {
            ...
            return (
              <div
                key={faq.q}
                className="bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-xs hover:border-[#0E7C93]/40 transition-colors"
              >
                <button
                  onClick={() => setOpenIndex(isOpen ? null : idx)}
                  className="w-full p-6 text-left flex items-center justify-between gap-4 focus:outline-none"
                >
                  <span className="text-base sm:text-lg font-bold text-[#1B3F60]">
                    {faq.q}
                  </span>
                  <div className={`p-2 rounded-full transition-all duration-200 shrink-0 ${
                    isOpen ? "bg-[#0E7C93] text-white rotate-180" : "bg-slate-100 text-slate-500"
                  }`}>
