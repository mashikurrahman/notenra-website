...
export function TestimonialsSection() {
  ...
  return (
    <section
      id="testimonials"
      className="py-16 sm:py-24 bg-slate-50 border-y border-slate-200 relative overflow-hidden"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-lg text-slate-600 mt-4 leading-relaxed font-normal">
            Real clinical leaders on how NOTENRA changed their documentation
            workflow.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((t, idx) => (
            <motion.div
              ...
              className="bg-slate-50 border border-slate-200 rounded-3xl p-8 flex flex-col justify-between hover:border-brand-teal-200 transition-all hover:border-brand-teal-200 group"
            >
              <div>
                {/* Rating Stars & Badge */}
                <div className="flex items-center justify-between mb-6">
                  <div className="flex items-center gap-1 text-brand-teal">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className="w-4 h-4 fill-brand-teal" />
                    ))}
                  </div>
              ...
              <div className="pt-6 border-t border-slate-200 flex items-center gap-4">
                ...
                <div className="w-12 h-12 rounded-2xl surface-navy text-white flex items-center justify-center font-extrabold text-sm shrink-0 shadow-xs">
                  {t.initials}
                </div>

                <div>
                  <div className="font-bold text-brand-ink text-sm flex items-center gap-1.5">
